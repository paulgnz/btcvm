...
// most UpTo satoshis needs Confirmations Bitcoin confirmations. Deposits above
// every tier need the bridge's full -confirmations. Waiting guards against
// a Bitcoin reorganisation undoing a deposit already credited, and the
...
// confirmationsFor is how many Bitcoin confirmations a deposit of value
// satoshis needs before it's credited.
...
// parseConfirmationTiers reads "10:6,50:12" (up to 10 BTC: 6
// confirmations; up to 50 BTC: 12). Tiers must rise in both amount and
...
func parseConfirmationTiers(s string, full int64) ([]confirmationTier, error) {
	...
	for _, part := range strings.Split(s, ",") {
		...
		if !ok {
			return nil, fmt.Errorf("confirmation tier %q is not BTC:CONFIRMATIONS", part)
		}
		upTo, err := parseBTC(amount)
		if err != nil {
...
func formatConfirmationTiers(tiers []confirmationTier) string {
	...
	for i, t := range tiers {
		parts[i] = strings.TrimRight(strings.TrimRight(formatBTC(t.UpTo), "0"), ".") + ":" + strconv.FormatInt(t.Confirmations, 10)
	}
...
// moment it lands, rather than on its next poll. BTCVM is checked four
// times a second, Bitcoin once a second.
func (b *bridge) waitForBlock(max time.Duration) {
	vm, vmOK := b.vm.(blockCounter)
	btc, btcOK := b.btc.(blockCounter)
	if !vmOK && !btcOK {
		time.Sleep(max)
		return
	}
	...
	startVM, startBTC := height(vm, vmOK), height(btc, btcOK)
	...
	for n := 1; time.Now().Before(deadline); n++ {
		...
		if n%4 == 0 {
			if h := height(btc, btcOK); h >= 0 && startBTC >= 0 && h != startBTC {
				return
			}
		}
	}
...
// tiersForAPI is the tiers with amounts as BTC strings, like every other
// amount the API returns.
func tiersForAPI(tiers []confirmationTier) []map[string]any {
	out := make([]map[string]any, len(tiers))
	for i, t := range tiers {
		out[i] = map[string]any{"upTo": formatBTC(t.UpTo), "confirmations": t.Confirmations}
	}
	return out
}
