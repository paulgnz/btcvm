...
// refresh the moment a block lands, instead of on a timer. On BTCVM a
...
// chainEvent says a chain has a new block, or the Bitcoin index has caught
// up with one.
type chainEvent struct {
	Chain  string // "btcvm" or "bitcoin"
	Height int64
}
...
// watchBlocks publishes an event for each new BTCVM block, checking
// four times a second, and for each Bitcoin block once the Bitcoin index
...
func (srv *server) watchBlocks(hub *eventHub) {
	var vmHeight, btcHeight int64 = -1, -1
	...
	for n := 0; ; n++ {
		...
		if err := srv.vm.rpc.call(&h, "getblockcount"); err == nil && h != vmHeight {
			...
			hub.publish(chainEvent{"btcvm", h})
		}
		if n%4 != 0 { // Bitcoin: once a second is plenty for one-minute blocks
			continue
		}
		var d int64 = -1
		if srv.btcIdx != nil {
			d, _ = srv.btcIdx.tip()
		} else if err := srv.btc.rpc.call(&d, "getblockcount"); err != nil {
			d = -1
		}
		if d >= 0 && d != btcHeight {
			first := btcHeight < 0
			...
			hub.publish(chainEvent{"bitcoin", d})
		}
	}
}
