...
PKG="github.com/MetalBlockchain/dogecoin-vm/btcd"
...
for i in $SYS; do
    ...
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" github.com/MetalBlockchain/dogecoin-vm/btcd
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" github.com/MetalBlockchain/dogecoin-vm/btcd/cmd/btcctl
    cd ..
